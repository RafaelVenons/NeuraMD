...
test.describe("PLAN note navigation", () => {
  test("opens an imported PLAN note and follows a sequential child link without server error", async ({ page }) => {
    const token = `${Date.now()}-${Math.random().toString(16).slice(2, 8)}`
    const scenario = runRailsScript("script/e2e/bootstrap_plan_navigation.rb", { E2E_TOKEN: token })

    await signIn(page, scenario.credentials)

    const response = await page.goto(scenario.target_note_path)
    ...
    await page.getByRole("link", { name: scenario.child_note_title }).click()
    await expect(page).toHaveURL(new RegExp(`${scenario.child_note_path.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`))
