...
class WorktreeService
  ...
  class << self
    def ensure(tentacle_id:, repo_root: Rails.root, worktree_root: nil, link_shared: true)
      ...
      # Best-effort: a fresh worktree should be able to boot bin/mcp-server
      # with the master.key + symlinks we just provisioned. If it can't,
      # log the failure (slug-less; SessionControl tags slug context) so
      # the next bin/start surfaces a real cause instead of "MCP failed".
      smoke_test_mcp_server!(path: path)
    ...
    # Emit `<worktree>/.claude/settings.local.json` granting bypass
    # permissions for operational agents (`tentacle_yolo: true` charters).
    # Required for unattended agents like Sentinela de Deploy: without
    # bypass, every `bin/brakeman` / `bin/rubocop` / etc. tool call from
    # a freshly-spawned worktree triggers a permission prompt that the
    # agent cannot answer, and the wake-up dies silently.
    #
    # Idempotent. Overwrites the file when contents drift but skips
    # writing when the desired payload already matches — so re-ensure
    # does not churn `mtime` unless something actually changed.
    YOLO_SETTINGS_PAYLOAD = {
      "permissions" => {"defaultMode" => "bypassPermissions"}
    }.freeze

    YOLO_SETTINGS_BANNER = "// Generated by WorktreeService — tentacle_yolo opt-in. Do not edit.\n".freeze

    def write_yolo_settings!(path:)
      path = Pathname.new(path)
      return unless path.directory?

      target = path.join(".claude/settings.local.json")
      desired = JSON.pretty_generate(YOLO_SETTINGS_PAYLOAD) + "\n"

      return if target.file? && target.read == desired
    ...
    # Boot bin/mcp-server in --check mode from inside the worktree to
    # verify the runtime can require config/environment + load the MCP
    # tool registry with the master.key/symlinks we just provisioned.
    # Failures are logged (path, exit_code, stderr_tail) but never
    # raised — the smoke is diagnostic only. Skips silently when the
    # target script does not exist (workspace-mode worktrees for
    # non-NeuraMD repos), or when NEURAMD_DISABLE_MCP_SMOKE=1.
    MCP_SMOKE_TIMEOUT_SECONDS = 5
    MCP_SMOKE_STDERR_TAIL_BYTES = 2_000

    def smoke_test_mcp_server!(path:)
      return if ENV["NEURAMD_DISABLE_MCP_SMOKE"].to_s == "1"

      script = Pathname.new(path).join("bin/mcp-server")
      return unless script.file?
      return unless File.executable?(script)

      out, status = Open3.capture2e(
        {"NEURAMD_TENTACLE_ID" => ""},
        "timeout", MCP_SMOKE_TIMEOUT_SECONDS.to_s,
        script.to_s, "--check",
        chdir: path.to_s
      )
      return if status.success?

      tail = out.to_s.byteslice(-MCP_SMOKE_STDERR_TAIL_BYTES, MCP_SMOKE_STDERR_TAIL_BYTES) || out.to_s
      Rails.logger.warn(
        "[worktree_service] mcp-server --check failed for #{path}: " \
        "exit=#{status.exitstatus.inspect} stderr_tail=#{tail.inspect}"
      ) if defined?(Rails)
    rescue StandardError => e
      Rails.logger.warn("[worktree_service] mcp-server smoke raised #{e.class}: #{e.message}") if defined?(Rails)
    end
