...
module Mcp
  module Tools
    class UpdateNoteTool < MCP::Tool
      ...
      def self.call(slug:, title: nil, content_markdown: nil, add_tags: nil, remove_tags: nil, append_links: nil, server_context: nil)
        note = find_note(slug)
        return error_response("Note not found: #{slug}") unless note

        has_changes = [title, content_markdown, add_tags, remove_tags, append_links].any?(&:present?)
        return error_response("Nothing to update — provide title, content_markdown, add_tags, remove_tags, or append_links") unless has_changes
