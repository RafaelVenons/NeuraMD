...
module Mcp
  module Tools
    class TaskHistoryTool < MCP::Tool
      tool_name "task_history"
      description <<~DESC.strip
        Últimas N tasks fechadas de um agente (closed_at IS NOT NULL). Ordem: closed_at desc. Sem auth. Default agent_slug = `ENV["NEURAMD_AGENT_SLUG"]`.
      DESC
      ...
      input_schema(
        ...
        properties: {
          ...
          limit: {type: "integer", description: "Máximo de tasks (default 20, max 100)"}
        }
      )
      ...
      def self.call(agent_slug: nil, limit: nil, server_context: nil)
        ...
        notes = Tasks::Protocol.task_history(agent_slug: slug, limit: bounded_limit)
